...
module.exports = {
    fetchAllReportingHeads: function(req,res){
        try {
            //connection.connect();
            console.log("Connection established for All Reporting Head details");
            ...
            connection.query("SELECT EmployeeId,IF(ISNULL(LastName), FirstName, CONCAT(FirstName, ' ', LastName)) AS ManagerName FROM employeemaster WHERE IsReportingHead = '1' and IsDeleted='0'", function(err, results) {
            if (err) throw err;
            if (results.length === 0) {
